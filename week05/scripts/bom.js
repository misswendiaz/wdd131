...
let chaptersArray = getChapterList() || []; // Initializes the chaptersArray variable with the list of chapters returned by the getChapterList() function or an empty array if the function call returns null or undefined
...
button.addEventListener("click", () => { // Adds an event listener to the button that listens for click events. When the button is clicked, the function inside will execute.
    if (input.value.trim() != "") { //Makes sure the input is not empty
...
function displaylist(item) {
    let li = document.createElement("li"); // Creates a new <li> (list item) element, but doesn't add it to the DOM yet. Prepares a new list item where the chapter name and delete button will go.
    let deleteButton = document.createElement("button"); // Creates a new <button> element to later be used for deleting the list item. Lets users remove chapters from the list.

    li.textContent = item; // Sets the text of the <li> element to the current value entered in the input box. This will be overridden soon by the next .append() calls unless structured properly.
    deleteButton.textContent = "❌"; // Sets the button’s label to a red cross emoji — a common delete symbol.
    li.append(deleteButton); // Adds the delete button inside the <li> so that it's part of the list item visually and functionally.
    list.append(li); // Adds the newly created <li> element as a child of the list element (e.g., a <ul> or <ol>). This uses append(), a modern DOM method that allows appending nodes or even strings.
